...
export function Navbar() {
  ...
  return (
    <>
      <aside
        className={cn(
          'fixed left-0 top-0 z-30 hidden h-screen flex-col border-r-4 border-[#07040b] bg-[#160827] text-white shadow-[10px_0_0_rgba(0,0,0,0.26)] transition-[width] duration-300 lg:flex',
          collapsed ? 'w-[5.5rem] p-3' : 'w-72 p-5',
        )}
      >
        ...
        <div className={cn('pixel-shell relative z-10 flex items-center justify-center', collapsed ? 'h-16 p-1' : 'h-24 p-3')}>
          {collapsed ? (
            <Image src={ui.shieldLogo} alt="SanData" width={58} height={58} className="h-12 w-12 object-contain" priority />
          ) : (
        ...
        <button
          ...
          className={cn(
            'relative z-10 mt-4 grid h-10 place-items-center border-2 border-[#0b0610] bg-[#28123f] text-gold shadow-[0_0_0_2px_#9b6427] transition hover:bg-[#3b185d]',
        ...
        <nav className={cn('relative z-10 mt-6 min-h-0 flex-1 space-y-2 overflow-y-auto thin-scrollbar', collapsed ? 'pr-0' : 'pr-1')}>
          {navItems.map((item) => {
            ...
            return (
              <Link
                ...
                href={item.href}
                className={cn(
                  'group relative flex items-center border-2 border-transparent py-2.5 text-sm font-black transition',
              ...
              >
                <Image src={item.icon} alt="" width={36} height={36} className="h-7 w-7 object-contain drop-shadow" />
        ...
        <div className={cn('pixel-panel relative z-10 mt-4', collapsed ? 'mx-auto h-12 w-12 p-1' : 'p-3')}>
          <div className={cn('flex items-center', collapsed ? 'justify-center' : 'gap-3')}>
            <Image src={ui.shieldLogo} alt="" width={56} height={56} className={cn('object-contain', collapsed ? 'h-9 w-9' : 'h-11 w-11')} />
            {!collapsed && (
              <div>
        ...
        <button
          ...
          className={cn(
            'relative z-10 mt-4 flex items-center justify-center border-2 border-white/15 bg-black/24 py-3 text-sm font-bold text-white hover:bg-white/14',
